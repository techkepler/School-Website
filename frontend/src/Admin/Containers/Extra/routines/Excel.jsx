  ...
  const handleSubmitBtn = async (e) => {
    ...
    try {
      let res = await axiosAdmin.post(
        "informations/routine/excel/file/",
        formData
      );
    ...
    <>
      ...
      {sucsMsg === 201 && (
        <div className="items-center w-full absolute  flex md:justify-center">
          <p className="px-4 text-sm md:text-base w-[90%]  md:w-[60%] top-0  py-2  text-slate-900 bg-green-500 mx-2 rounded-md fixed text-center z-40">
            Routines Added Successfully.
          </p>
        </div>
      )}
      ...
      {errMsg && (
        <div className="items-center w-full absolute  flex md:justify-center">
          <p className="px-4 w-[90%]  md:w-[60%] top-0  py-2  text-slate-900 bg-red-500 mx-2 rounded-md fixed text-center z-40 text-sm md:text-base">
