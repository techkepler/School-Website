...
const ViewImage = () => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      let res = await axiosAdmin.get("informations/crud/gallery/?page_size=20");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `informations/crud/gallery/?page=${currentPage}&page_size=20`
    );
    setTotalData(res.data.count);
    setInfoData(res.data.results);
  };
  ...
  const filterData = () => {
    let filterImg = infoData;
    if (searchQuery) {
      filterImg = infoData.filter((data) =>
  ...
  const yesBtnClick = async () => {
    ...
    let res = await axiosAdmin.delete(
      `informations/crud/gallery/${deleteData}/`
    );
    ...
    <>
      ...
      >
        <input
          ...
          placeholder="Search image name here..............."
            ...
            <table className="w-full">
              <thead>
                <tr className=" text-sky-700  dark:text-green-500 bg-slate-300 dark:bg-slate-700 border border-slate-300 dark:border-slate-800 pb-4">
                  <th className="whitespace-nowrap p-5   ">Image</th>
              ...
              <tbody>
                {filterData().length > 0 &&
                  filterData().map((datas, index) => (
                    <tr
                      key={index}
                      className="  text-slate-700 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 border border-slate-300 dark:border-slate-800 pb-4"
                    >
                      <td className="whitespace-nowrap px-2 sm:px-8 py-5 text-sm capitalize text-center">
                        <img
                          src={datas.image}
                          alt={datas.name}
                          className="w-20 h-20 md:h-24 md:w-24"
                        />
                      </td>

                      <td className="whitespace-nowrap  px-8 py-5 text-sm capitalize text-center font-semibold">
                        {datas.name}
                      </td>
      ...
      >
        <div className="w-80 md:w-96 h-36 bg-slate-300  px-4 py-4 rounded-md flex flex-col justify-between">
          <p className="">Are you sure you want to delete this image?</p>
