...
import SearchTeacherSalary from "./SearchFee";
...
const ViewStuFee = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get("students/crud/fee/?page_size=50");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `students/crud/fee/?page=${currentPage}&page_size=50`
    );
    setTotalData(res.data.count);
    setStudentData(res.data.results);
  };
  ...
  const yesBtnClick = async () => {
    setIsDel(false);
    let res = await axiosAdmin.delete(`students/crud/fee/${deleteData}/`);
          ...
          {filterData().length > 0 && (
            <div className="overflow-auto my-10 bg-slate-100 rounded-md dark:bg-slate-900  box-border shadow-2xl">
              <table className="w-full">
                <thead>
                  <tr className="  text-slate-700 dark:text-[#05dcf8] hover:bg-gray-300 dark:hover:bg-slate-700 border-b-[1px] border-slate-300 dark:border-slate-800 pb-4">
                  ...
                  {filterData().length > 0 &&
                    filterData().map((datas, index) => (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap  px-8 py-5 text-sm text-center">
                          <Link to={`/admin/students/fee/edit/${datas.id}/`}>
