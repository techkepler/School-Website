...
const EditStudentAttendance = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let res = await axiosAdmin.get(`students/crud/attendance/${id}/`);
  ...
  const handleSubmitBtn = async (e) => {
    e.preventDefault();
    try {
      let res = await axiosAdmin.put(`students/crud/attendance/${id}/`, {
        id: id,
        attend_day: attend,
        absent_day: absent,
        ...
        {sucsMsg === 200 && (
          <div className="items-center w-full absolute  flex md:justify-center">
            <p className="px-4 text-sm md:text-base w-[90%] md:w-[60%] top-0 py-2  text-gray-900 bg-green-500 mx-2 rounded-md fixed text-center z-40">
              Student Attendnace Updated Successfully.
            </p>
          </div>
        )}
        {errMsg.msg && (
