...
const ViewStudent = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get("students/crud/student/?page_size=50");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `students/crud/student/?page=${currentPage}&page_size=50`
    );
    setTotalData(res.data.count);
    setTeacherData(res.data.results);
  };
  ...
  const yesBtnClick = async () => {
    setIsDel(false);
    let res = await axiosAdmin.delete(`students/crud/student/${deleteData}/`);
                  ...
                  <tbody>
                    {filterData().length > 0 &&
                      filterData().map((datas, index) => (
                        ...
                        >
                          <td className="whitespace-nowrap p-5 text-sm  my-4">
                            {datas.id}
                          </td>
                              ...
                              <button
                                className={`${
                                  datas.gender === "Male"
                                    ? "bg-green-600"
                                    : "bg-sky-600"
                                } rounded-md px-2 py-1 text-sm text-slate-100 capitalize`}
                                disabled
                              >
                                {datas.gender}
                              </button>
                          ...
                          <td className="whitespace-nowrap  p-5 text-sm">
                            <Link to={`/admin/students/edit/${datas.id}/`}>
                    ...
                    filterData().map((datas, index) => (
                      ...
                      >
                        ...
                        <div className="member-details flex gap-1 flex-col items-center dark:text-slate-300 text-slate-800">
                          ...
                          <p className="text-sm capitalize">{datas.grade}</p>
