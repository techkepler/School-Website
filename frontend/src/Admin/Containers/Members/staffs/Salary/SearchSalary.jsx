...
const SerachTeacherSalary = () => {
  ...
  const handleSubmitBtn = async (e) => {
    e.preventDefault();
    let res = await axiosAdmin.get(`staffs/crud/salary/?id=${id}&name=${name}`);
  ...
  const yesBtnClick = async () => {
    setIsDel(false);
    let res = await axiosAdmin.delete(`staffs/crud/salary/${deleteData}/`);
    let newResponse = await axiosAdmin.get(
      `staffs/crud/salary/?id=${id}&name=${name}`
    );
  ...
  return (
    <>
      {/* Response Msg */}
      {sucsMsg === 204 && (
        <div className="items-center w-full absolute  flex md:text-center">
          <p className="px-4 text-sm md:text-base w-[90%]  md:w-[60%] top-0  py-2  text-gray-900 bg-green-500  rounded-md fixed text-center z-40">
            Staff Salary Deleted Successfully.
          </p>
        </div>
      )}
              ...
              {responseData.length > 0 &&
                responseData.map((datas, index) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap  p-5 text-sm text-center">
                      <Link to={`/admin/staffs/salary/edit/${datas.id}/`}>
...
export default SerachTeacherSalary;
