...
const ViewTeacherAttendance = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get("teachers/crud/attendance/?page_size=50");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `teachers/crud/attendance/?page=${currentPage}&page_size=50`
    );
    setTotalData(res.data.count);
    setTeacherData(res.data.results);
  };
  ...
  const yesBtnClick = async () => {
    ...
    let res = await axiosAdmin.delete(
      `teachers/crud/attendance/${deleteData}/`
    );
  ...
  return (
    <>
      <div>
                    ...
                    {filterData().length > 0 &&
                      filterData().map((datas, index) => (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap  p-5 text-sm text-center">
                            <Link
                              to={`/admin/teachers/attendance/edit/${datas.id}/`}
                            >
        ...
        {/* Teacher Search */}
        {searchTeacher && <SearchTeacherAttendance />}
