...
const ViewTeachers = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get("teachers/crud/teacher/?page_size=50");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `teachers/crud/teacher/?page=${currentPage}&page_size=50`
    );
    setTotalData(res.data.count);
    setTeacherData(res.data.results);
  };
                    ...
                    filterData().map((datas, index) => (
                      ...
                      >
                        ...
                        <div className="member-details flex gap-1 flex-col items-center dark:text-slate-300 text-slate-800">
                          ...
                          <p className="text-sm capitalize">{datas.subject}</p>
