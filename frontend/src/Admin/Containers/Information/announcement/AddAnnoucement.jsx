...
const AddAnnouncement = ({ isViewInfoClick }) => {
  ...
  const handleFormSubmit = async () => {
    setSubmit(false);
    try {
      let res = await axiosAdmin.post("informations/crud/announcement/", {
        title: title,
        data: data,
        status: status,
      });
    ...
    <div className="my-10 flex justify-center items-center">
        ...
        <form className="w-full my-4 pb-5 md:px-6 flex flex-col items-center justify-center ">
          <div className="w-full flex flex-col gap-8  justify-center my-4 ">
            ...
            <div>
              ...
              >
                <span className="text-red-500 mr-1">*</span>
                <span style={{ color: themeColor }}>Write announcement:</span>
              </label>
              <Editor
                apiKey="i5ld1nbpql0sgsemjrfpc30fecthuzsmgb3sgataomu0mzyx"
                onInit={(evt, editor) => (editorRef.current = editor)}
                initialValue="Write announcement here............"
                init={{
                  height: 500,
                  menubar: false,
                  plugins: [
      ...
      >
        <div className="w-80 md:w-96 h-36 bg-slate-300  px-4 py-4 rounded-md flex flex-col justify-between">
          <p className="">Are you sure you want to submit this announcement?</p>
