...
const ViewBlogs = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get("informations/crud/blogs/?page_size=20");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `informations/crud/blogs/?page=${currentPage}&page_size=20`
    );
    setTotalData(res.data.count);
    setInfoData(res.data.results);
  };
  ...
  const yesBtnClick = async () => {
    setIsDel(false);
    let res = await axiosAdmin.delete(`informations/crud/blogs/${deleteData}/`);
                ...
                {filterData().length > 0 &&
                  filterData().map((datas, index) => (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap  px-8 py-5 text-sm text-center">
                        <Link to={`/admin/blogs/edit/${datas.slug}/`}>
