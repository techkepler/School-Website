...
import SearchTeacherAttendance from "./SearchAttend";
...
const ViewTeacherAttendance = () => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      let res = await axiosAdmin.get("students/crud/attendance/?page_size=50");
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `students/crud/attendance/?page=${currentPage}&page_size=50`
    );
    setTotalData(res.data.count);
    setStudentData(res.data.results);
  };
  ...
  const filterData = () => {
    let fitlerTeachers = studentData;
    if (searchQuery) {
      fitlerTeachers = studentData.filter((data) =>
  ...
  const yesBtnClick = async () => {
    ...
    let res = await axiosAdmin.delete(
      `students/crud/attendance/${deleteData}/`
    );
    ...
    <>
      <div>
        ...
        <div
          ...
          } ${searchStudent ? "md:justify-center" : "md:justify-between "}`}
        >
          <p className={` order-2 md:order-1 ${searchStudent && "hidden"}`}>
            <input
              type="search"
              name="search"
              id="search"
            ...
            {filterData().length > 0 && (
              <div className="overflow-auto my-10 bg-slate-100 rounded-md dark:bg-slate-900  box-border shadow-2xl">
                <table className="w-full">
                  <thead>
                    <tr className="  text-slate-700 dark:text-[#9bbae7] bg-slate-300 dark:hover:bg-slate-700 border-b-[1px] border-slate-300 dark:border-slate-800 pb-4">
                    ...
                    {filterData().length > 0 &&
                      filterData().map((datas, index) => (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap  p-5 text-sm text-center">
                            <Link
                              to={`/admin/students/attendance/edit/${datas.id}/`}
                            >
        ...
        {/* Teacher Search */}
        {searchStudent && <SearchTeacherAttendance />}
