...
const ViewApplication = () => {
  ...
  useEffect(() => {
    const fetchTeacher = async () => {
      let res = await axiosAdmin.get(
        `informations/crud/leave/application/?stu_id=${auth.uid}`
      );
  ...
  const fetchNewData = async (currentPage) => {
    let res = await axiosAdmin.get(
      `informations/crud/leave/application/?stu_id=${auth.uid}&page=${currentPage}&page_size=20`
    );
    setTotalData(res.data.count);
    setInfoData(res.data.results);
  };
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap  px-8 py-5 text-sm text-center">
                        <div className="flex justify-center items-center gap-6">
                          <Link
                            to={`/student/leave/application/view/${datas.id}/`}
                          >
                            <FaEye className="text-lg inline  text-green-500 cursor-pointer" />
                          </Link>
                          <Link
                            to={`/student/edit/leave/application/${datas.id}/`}
                          >
                            <FaEdit className="text-lg inline  text-sky-500 cursor-pointer" />
                          </Link>

                          <VscTrash
                            className="text-lg text-red-500  inline cursor-pointer"
                            onClick={() => {
